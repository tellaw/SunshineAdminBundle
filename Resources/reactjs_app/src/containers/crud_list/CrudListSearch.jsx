import React from 'react';
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';

import { fetchList } from '../../actions/action_crud_list.jsx';

class CrudListSearch extends React.Component {

    constructor(props) {
        super(props);
        this.handleSearch = this.handleSearch.bind(this);
        this.handleChange = this.handleChange.bind(this);
        this.state = {value: ''};
    }

    handleSearch() {
        //this.props.fetchList()
        console.log(this.props.context);
        console.log(this.props.crudList.context);
        var entityName = this.props.crudList.context.entityName;
        var keyword = this.state.value;
        this.props.fetchList(entityName, 0, 10, keyword);
    }

    handleChange ( event ) {
        this.setState({value: event.target.value});
    }

    render() {
        return (
            <div>
                <input type="text" id="search" value={this.state.value} onChange={this.handleChange} />
                <button onClick={this.handleSearch} id="btn-search">OK</button>
            </div>
        );
    }

}

function mapDispatchToProps(dispatch) {
    return bindActionCreators({ fetchList }, dispatch);
}

function mapStateToProps({ context, crudList }) {
    return { context, crudList };
}

export default connect(mapStateToProps, mapDispatchToProps)(CrudListSearch);
