...
tinymce.PluginManager.add('bootstrap', function(editor, url) {
    ...
    /**
     * refs #4914
     * Quand on vide le contenu d'une div bootstrap, tinyMCE supprime la div "col-sm-x".
     * On va donc chercher à zapper cette suppression via le code suivant :
     * **/
    tinymce.activeEditor.on("keydown",function(e) {
        //prevent empty panels
        if (e.keyCode == 8 || e.keyCode == 46) { //backspace and delete keycodes
            try {
                var elem = tinymce.activeEditor.selection.getNode().parentNode; //current caret node
                if (elem.parentNode.classList.contains("row")) {
